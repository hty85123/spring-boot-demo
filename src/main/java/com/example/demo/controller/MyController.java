...
//        List<Student> students = studentRepository.findAllByOrderByGradeDesc();
        List<Student> students = foo();
        return ResponseEntity.ok(students);
    }
    ...
    private List<Student> foo() {
        ...
        Criteria gradeCriteria = Criteria.where("grade").gte(3);
        Criteria allCriteria = new Criteria().orOperator(List.of(gradeCriteria, certCriteria));
        Pageable pageable = PageRequest.of(1,2);
        Query query = new Query(allCriteria).with(pageable).with(Sort.unsorted());
